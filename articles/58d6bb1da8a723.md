...
# Storybook
[Storybook](https://storybook.js.org/) は UI コンポーネント単体で表示させることができるツールです。Jest ではコードベースのテストは行い、Storybookではビジュアルのテストを行います。特にデザイナーや PdM などエンジニア以外に見せる際にも有効です。
...
- spell checker の辞書登録。一般的ではないが開発上よく使うスペルを登録する。
